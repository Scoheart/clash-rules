...
4. **Apply Changes**:
   - **Update @clash-mihono.yaml**: Insert the new rule using `replace_file_content` or `multi_replace_file_content`.
     - **Format**: `      - DOMAIN,domain.com # Comment` (Ensure correct indentation).
   - **Update @shadowrocket.conf**: Insert the new rule using `replace_file_content` or `multi_replace_file_content`.
